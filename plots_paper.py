...
class App(wx.Frame):
    ...
    def __init__(self, parent, id, title):
                ...
                if not (ext == 'csv' or ext == 'DAT'or ext == 'SPE'): #拡張子がcsvであるかの判定
                    ...
                    if len(l)==1 and l[0] ==[]: #ファイルの配列がない場合
                        ...
                        if checkbox_7.GetValue():
                            M = mean(li_y[:int(len(li_y)/8)])
                            m = mean(li_y[int(len(li_y)/8*7):])
                            h = (M-m)*0.1+m
                            H = (M-m)*0.9+m
                            list_0 = [H]*len(li_x)
                            list_h = [h]*len(li_x)
                            d_y = [abs(i-h) for i in li_y]
                            d_Y = [abs(i-H) for i in li_y]
                            m1 = d_y.index(min(d_y))
                            m2 = d_Y.index(min(d_Y))
                            beam = abs(li_x[m1]-li_x[m2])
                            plt.plot(li_x,list_0,color=cmap(c),label = '0',linestyle=':')
                            plt.plot(li_x,list_h,color=cmap(c),label = '1/2',linestyle=':')
                            wx.MessageBox('ファイル名:'+basename+'\n'+'90%値:　　{0: .3e}'.format(H)+'\n'+'10%値:　　{0: .3e}'.format(h)+'\n'+'半値:　　　{0: .3e}'.format(m+h)+'\n'+'ビーム幅:　{0: .3e}'.format(beam*1.561)+'\n')
        ...
        checkbox_7 = wx.CheckBox(p, wx.ID_ANY,'ナイフエッジ')
